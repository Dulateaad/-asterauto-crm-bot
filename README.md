...
**Назначение лидов по бренду:** у менеджера в `ltbUsers` поле **`brands`** (массив). Лид уходит в пул менеджеров с этим брендом; иначе — в пул **без** `brands` (универсальные). Добавление: `/adduser 1850222787 manager Имя -- Changan` **или** `/adduser 1850222787 manager Имя` без `--` → бот пришлёт кнопки выбора брендов и «Все бренды».
